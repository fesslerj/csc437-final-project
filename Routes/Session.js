...
// it back
// as a cookie, creating a Session for |user|, and saving it in |sessions|
// indexed
// by the authToken.
//
// 1 Cookie is tagged by |cookieName|, times out on the client side after
// |duration|
// (though the router, below, will check anyway to prevent hacking), and will
// not be
...
exports.makeSession = function makeSession(user, res) {
   ...
   var authToken = crypto.randomBytes(cryptRandBytesLen).toString('hex');
...
exports.router = function(req, res, next) {
   // If we present a session cookie that corresponds with one
   // in |sessions|...
